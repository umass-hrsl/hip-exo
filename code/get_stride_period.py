...
def get_stride_period(fxs, baud_rate, time_step=1/100):
	times = []
	i = 0
	mocap_start = 0
	stop= 0
	...
	while True:
		if (GPIO.input(7) == GPIO.LOW) and (mocap_start == 0):
			continue
		elif (GPIO.input(7) == GPIO.LOW) and (mocap_start == 1):
			vel_filtered_0 = np.array([mot_vel_0[0]])
			start_count = 0
			stride_count = np.array(0)
			current_stride = 0
			print(vel_filtered_0)
			for i in range(1, len(mot_vel_0)):
				if i <= 26:
					vel_filtered_0 = np.append(vel_filtered_0 , np.mean(mot_vel_0[0:i]))
				elif len(mot_vel_0) - i <=  26:
					vel_filtered_0 = np.append(vel_filtered_0, np.mean(mot_vel_0[i-25:len(mot_vel_0)]))
				else:
					vel_filtered_0 = np.append(vel_filtered_0, np.mean(mot_vel_0[i-25:i+26]))
			...
			for i in (stride_count[-1]//2-15, stride_count[-1]//2+15):
				ind = np.where(stride_count == i)[0]
				if i == stride_count[-1]//2-15:
...
def main():
	...
	get_stride_period(flex.FlexSEA(), args.baud_rate)
