...
def bilateral_impedance_experiment(fxs, baud_rate, exp_time = 5*60, time_step=1/100):
	...
	currents=np.zeros((num_time_steps,1))
	...
	print(zero_setpoint_0)
	print(zero_setpoint_1)
	fxs.send_motor_command(dev_id_0, fxe.FX_IMPEDANCE, zero_setpoint_0)
		...
		try:
			...
			if (curr_time > on_time) and (curr_time <= on_time + 5):
				if ramp_up == 0:
					...
					ramp_up = 1
				stiffness_0=round(2000/5*(curr_time - on_time)*(2*math.pi/(0.0007812*16384*0.146*6))) #mNm/rad  6000 is 3360
				stiffness_1=-1000/5*(curr_time - on_time)
				damping_1= 10 #(mNm*s)/rad
				fxs.set_gains(dev_id_0, 40, 400, 0, stiffness_0, 0, 96)
			elif (curr_time > on_time + 5) and (curr_time <= off_time - 5):
				if constant == 0:
					print('Stiffness is constant')
					constant = 1
					stiffness_0=round(2000*(2*math.pi/(0.0007812*16384*0.146*6)))#mNm/rad
					fxs.set_gains(dev_id_0, 40, 400, 0, stiffness_0, 0, 96)
					print(stiffness_0)
				stiffness_1=-1000
				damping_1=10 #(mNm*s)/rad
				...
				if ramp_down == 0:
					...
					ramp_down = 1
				stiffness_0=round(-2000/5*(curr_time - off_time)*(2*math.pi/(0.0007812*16384*0.146*6))) #mNm/rad
			...
			motor_torque_0=stiffness_0*mot_ang_rad_0/(2*math.pi/(0.0007812*16384*0.146*6))
			...
			#Check if the difference between the time steps are huge, send the previous torque
			#if abs(torque[-1, 0] - motor_torque_0) > 5000:
			#	motor_torque_0 = torque[-1, 0]
			#	current_0 = currents[-1, 0]
			#if abs(torque[-1, 1] - motor_torque_1) > 5000:
			#	motor_torque_1 = torque[-1, 1]
			#	current_1 = currents[-1, 1]
	...
	file1=open("troubleshooting1.txt","a+")
	file1.truncate(0)
	file1.write("time position_rad_0 position_rad_1 command_current_0 motor_current_0 motor_current_1 velocity_rad_0 velocity_rad_1 velocity_filtered_0 velocity_filtered_1 torque_0 torque_1 stride_count\n")
	file1.flush()
	for index in range(len(times)):
		file1.write(str(times[index])+" "+str(ang_rad[index, 0])+" "+str(ang_rad[index, 1])+" "+str(currents[index, 0])+" "+str(motor_curr[index, 0])+" "+str(motor_curr[index, 1])+" "+str(velocities_rad[index, 0])+" "+str(velocities_rad[index, 1])+" "+str(velocities_rad_avg[index, 0])+" "+str(velocities_rad_avg[index, 1])+" "+str(torque[index, 0])+" "+str(torque[index, 1])+" "+str(stride_count[index])+"\n" )
	file1.flush()
	file1.close()
	...
	plt.figure()
	plt.plot(times, currents[:,0], color="b",label="commanded current 1")
	plt.xlabel("time (s)")
	plt.legend(loc="upper right")
	...
	plt.figure()
	plt.plot(times, velocities_rad_avg[:,0], color="r",label="vel filtered 0")
	plt.xlabel("time (s)")
	plt.legend(loc="upper right")

	plt.figure()
	plt.plot(times, velocities_rad_avg[:,1], color="r",label="vel filtered 1")
	plt.xlabel("time (s)")
	plt.legend(loc="upper right")
